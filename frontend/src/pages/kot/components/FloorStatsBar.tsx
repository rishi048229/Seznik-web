...
export const FloorStatsBar = ({ total, occupied, vacant, revenue }: FloorStatsBarProps) => {
  const stats = [
    ...
    { label: 'Active Dining', value: formatINR(revenue), icon: IndianRupee, accent: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30' },
  ]
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
      {stats.map(({ label, value, icon: Icon, accent }) => (
        <div
          key={label}
          className="flex items-center gap-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm px-3.5 py-3 shadow-sm"
        >
