...
export const createTokenType = async (data: { name: string; price: number | null; taxRate?: number }): Promise<TokenType> => {
  return await fetchApi('/token-types', {
    method: 'POST',
...
export const updateTokenType = async (
  tokenTypeId: string,
  data: Partial<{ name: string; price: number | null; taxRate: number; sortOrder: number; isActive: boolean }>
): Promise<void> => {
